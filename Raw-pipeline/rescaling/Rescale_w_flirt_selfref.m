...
%% User input
%Set FSL environment
run('/Volumes/pmc_mpartanen/MRIdata/Code/CopyPasteRename/set_fsl_ev.m'); 
...
subs = dir(fullfile(copy_folder, 'sub-*'));
subs_names = {subs.name}'; %names only s
...
for i=1:length(subs_names)
    ...
    ses = dir(fullfile(copy_folder, subs_names{i}, 'ses-*'));
    ...
    for k=1:length(ses_names)
        ...
        if ~isempty(filename_to_rescale_15)
            ...
            full_name_output_15 = fullfile(derivatives_folder, subs_names{i}, [temp_rescale_15_names(1:end-4) '_rescale_1x1x1.nii']);
            xcmd_15 = ['flirt -in ' full_name_input_15 ' -ref ' full_name_input_15 ' -applyisoxfm 1.0 -nosearch -out ' full_name_output_15]; %reference is specified in the user input section
            disp(full_name_input_15); disp(full_name_output_15);
            [status,result] = system(xcmd_15); %copy with Linux cp, matlab is too slow.
        end

        filename_to_rescale_3 = dir(fullfile(copy_folder, subs_names{i}, ses_names{k}, 'anat', '*s_T1_3D_TFE_2*.nii')); %full name of the 3T file that needs to be rescaled.
        if ~isempty(filename_to_rescale_3)
            ...
            full_name_output_3 = fullfile(derivatives_folder, subs_names{i}, [temp_rescale_3_names(1:end-4) '_rescale_1x1x1.nii']);
            xcmd_3 = ['flirt -in ' full_name_input_3 ' -ref ' full_name_input_3 ' -applyisoxfm 1.0 -nosearch -out ' full_name_output_3]; %reference is specified in the user input section
