...
output_root = '/Volumes/pmc_mpartanen/MRIdata/SuSPeCT_Diffusion';
addpath(genpath('/Users/brunorobalo/Documents/MATLAB/spm12'));
addpath(genpath('/Users/brunorobalo/Documents/NEUROIMAGING/MRIToolkit-master'));
...
RawFolder = fullfile(output_root,'Brain_nii');
OutputFolder = fullfile(output_root,'Brain_BIDS');
...
for subj_id=41:length(SubjectsFolder) %run from 1 to 45 with
        ...
        if(~isempty(json_files))
            for json_id=1:length(json_files)
                ...
                for cate = 1:length(AcquisitionCategories)
                    ...
                    tempCatname1 = strrep(tempCatname, ' ', '_'); %%%%%%%%%%%%%%%% 
                    ...
                    category_exist1 = strfind(acq_category,tempCatname1); %search for acquisition categorie tags in json file

                    %if it finds the categorie, atribute a folder 'anat',
                    %func or dw
                    if ~isempty(category_exist) || ~isempty(category_exist1)
...
%                                 copyfile(fullfile(full_sub_ses_path, json_file(njson).name), TempDirectory)
                                ...
                                if strcmp(tempOutFolder,'dwi')  && ~isempty(temp_exist_bvec) && ~isempty(temp_exist_bval)
